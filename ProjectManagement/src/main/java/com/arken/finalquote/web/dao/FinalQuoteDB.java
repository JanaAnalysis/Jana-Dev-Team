	...
	public static int SaveFinalQuote(FinalQuoteBean fqb) throws SQLException
	{
		...
		try
		{
			...
			ps=con.prepareStatement("INSERT INTO `arken`.`final_quote`(`project_id`,`quote_id`,`s_no`,`description`,`model`,`qty`,`units`,`unitprice`,`totalprice`,"
					+ "`insunitprice`,`instotalprice`,`date`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
		  ...
		  try
	        {
			  ...
			  ps = con.prepareStatement("SELECT s_no,item_name,model,qty,units,unitprice,totalprice,insunitprice,instotalprice,date FROM arken.final_quote,arken.items where final_quote.description=items.itemid and project_id=?;");
			  ps.setInt(1, pid);
			  rs1 = ps.executeQuery();
